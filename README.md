...
1. **POM (Page Object Model) 设计模式**：高度解耦。将页面元素定位与测试逻辑分离，当 UI 发生变化时，只需修改对应的 Page 类，无需变动测试脚本。* **工程化底层封装**：
...
2. **数据驱动测试 (DDT)**：利用 `PyYAML` 加载 `test_data/users.yaml` 中的多账号数据，实现一套代码验证标准用户、锁定用户、问题用户等多种场景。
3. **高可用的报告集成**：
    * **Allure Report**: 自动生成结构化的测试报告。
    * **失败自动截图**: 在 `conftest.py` 中利用 Pytest 钩子函数，在用例断言失败时自动捕获屏幕并嵌入 Allure 报告中。
4. **彩色日志系统**: 集成 `colorlog`，支持控制台彩色输出并同步保存至本地 `logs/` 目录，便于快速定位执行过程。
